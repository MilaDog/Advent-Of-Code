...
def get_day_input(day: str, year: str) -> str:
...
def create_input_file(year: str, day: str) -> None:
    ...
    inputs_path: Path = Path(f"./inputs/{year}/{str(day).zfill(2)}")

    if not inputs_path.exists():
        inputs_path.mkdir(parents=True, exist_ok=True)
        print(f"Created inputs folder: `{inputs_path}`.")

    if not os.path.isfile(inputs_path / "input.txt"):
        with open(inputs_path / "input.txt", "w") as file:
            file.write(get_day_input(day=day, year=year))

        print(f"Created data file: `{inputs_path / 'input.txt'}`.")
